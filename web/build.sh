#!/bin/sh
# Build Rogue 5.4 for the browser (Emscripten + Asyncify) into web/dist;
...
emcc -O2 -fcommon -std=gnu89 -w -Wno-error=return-mismatch -Wno-error=implicit-function-declaration -Wno-error=implicit-int -Wno-error=int-conversion -Wno-error=incompatible-pointer-types -Iport -DWC_STATUS_ROWS=1 -DHAVE_CONFIG_H \
	$SRCS port/wcurses.c port/tiles.c port/be_web.c \
	-o "$OUT/rogue54-core.js" \
...
cp web/index.html "$HOME/Games/rvip-tools/web/rvip-wm.js" web/rogue54.js port/tiles.png port/tiles-dawn.png "$OUT/"
