/*
 * Rogue 5.4 in the browser: draws the panes of the curses shim (port/be_web.c
 * calls Module.xr), keyboard input, tiling windows, sound, saves in
 * IndexedDB. Loaded before rogue54-core.js.
 */
(function () {
	'use strict';
	...
	var DIR = '/rogue54/save';                         /* IDBFS mount: saves, scores, layout */
	...
	function crashed(err) {
		...
		console.error('[rogue54] crash:', err);
	...
	window.addEventListener('error', function (e) {
		...
		if (e.error instanceof WebAssembly.RuntimeError || /rogue54-core/.test(e.filename || '')) crashed(e.error || e.message);
	});
